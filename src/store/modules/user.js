...
const mutations = {
  ...
  // 设置当前链信息
  SET_CURRENT_CHAIN_INFO(state, chainInfo) {
    saveCurrentChainInfo(chainInfo)
    state.currentChainInfo = chainInfo
  },
  // 移除当前链信息
  REMOVE_CURRENT_CHAIN_INFO(state) {
    saveCurrentChainInfo('')
    state.currentChainInfo = null
  },
  // 设置可以进入的联盟列表
...
const actions = {
  ...
  logout({ commit, state, dispatch }) {
    return new Promise((resolve, reject) => {
      ...
      commit('REMOVE_CURRENT_CHAIN_INFO')
...
function saveCurrentChainInfo(chainInfo) {
  // storage.temp('local', 'currentChainInfo', chainInfo)
}

function getCurrentChainInfo() {
  // return storage.temp('local', 'currentChainInfo')
}
